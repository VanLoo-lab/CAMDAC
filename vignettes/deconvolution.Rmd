...
- `CamConfig()` sets the configuration for camdac pipeline functions.
- `CamSample()` sets details for each biological sample.
...
1. The directory passed when creating the CAMDAC config object (see camdac_refs argument in `CamConfig()`)

...
If for any reason, you have changed the output directory structure from previous run, you can initiate CAMDAC by manually passing outputs to `CamSample` objects. See the vignette `vignettes("output")` for more information.

Finally, you can run the `cmain_*` functions used internally by `pipeline` directly. These functions are not exported, but can be accessed with `:::`. For example, to run the deconvolution step, you can call `CAMDAC:::cmain_deconvolution`.
...
## Can I manually override specific pipeline outputs?

...
format_output <- function (patient_id,sample_id,sex,
  ...
  if(sum(flag)<length(flag)){dt_combined[,] <- dt_combined[flag,]}
  rm(flag)
  
  # Ensure that spurious alignments to Y in females are removed
  if(sex=="XX"){dt_combined<-dt_combined[!dt_combined$chrom%in%c("Y","chrY"),]}
...
get_msp1_fragments <- function(dt,build,path_to_CAMDAC,outfile){
  ...
  fragments_patient <- foverlaps(dt_CCGGs, fragments, nomatch=0);rm(dt_CCGGs)
  ...
  overlaps <- foverlaps(unknown_fragments, fragments, nomatch=0);rm(fragments)
  ...
  fragments_patient <- rbind(fragments_patient, overlaps);rm(overlaps)

...
annotate_bam_with_loci <- function(bam_dt, loci_subset, drop_ccgg=FALSE, paired_end=FALSE){
  ...
  if(drop_ccgg){ loci_subset <- loci_subset[width!=4]}
...
# Remove reads that do not cover entire dinucleotide
...
filter_bad_allele_count_rows <- function(pileup_summary, min_cov){
  ...
  loci_low_unexpected <- nrow(pileup_summary); loci_high_unexpected <- loci_all - loci_low_unexpected
...
format_get_reads_result <- function(dt){
  if(nrow(dt)==0){ return(empty_count_alleles_result())}
...
#' @export
cwrap_get_allele_counts <- function(
  bam_file, seg, loci_dt=NA, paired_end, drop_ccgg, min_mapq=1, min_cov=3
){
  ...
  if(nrow(bam_dt)==0){ return(empty_count_alleles_result())}
  ...
  pileup_summary <- flatten_pileup_to_counts(bam_dt); rm(bam_dt)

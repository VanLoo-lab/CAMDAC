...
unique_calculate_counts_hdi <- function(M, UM, n_cores=1, itersplit=5e5){
  ...
  #Validate M-length
  inp_len = length(M); stopifnot(inp_len == length(UM))
  
  # Get unique pairs to save computation
  udata = unique(data.table(M=M, UM=UM))
  M=udata$M; UM=udata$UM; inp_len=length(M); rm(udata)
...
process_methylation <- function(allele_counts, min_meth_loci_reads=3){
  ...
  methyl <- allele_counts[ width>1 & !is.na(m) & total_counts_m > min_meth_loci_reads]; rm(allele_counts)
...
combine_tumour_normal_methylation <- function(t_meth, n_meth){
  ...
  meth_c$i.start=NULL; meth_c$i.end=NULL
  setkey(meth_c, chrom, start, end)
  return(meth_c)
}

annotate_cgs_with_cnas <- function(meth_c, tumour, cna){
  cna$cna$chrom = factor(cna$cna$chrom, levels=c(1:22, "X", "Y"))
  setkey(cna$cna, chrom, start, end)
  meth_cna = foverlaps(cna$cna, meth_c, nomatch=0)
  meth_cna[, i.start:=NULL]; meth_cna[, i.end:=NULL]
  names(meth_cna)[which(names(meth_cna)=="seg_max")] = "seg_start"
  names(meth_cna)[which(names(meth_cna)=="seg_min")] = "seg_end"
...
HDIofMCMC_mt = function(M_b,UM_b,M_n,UM_n,p,CN,credMass=0.99) {
  ...
  # Return HDI
  HDIlim <- (c(lower = mt_dist[which.min(ciWidth)], 

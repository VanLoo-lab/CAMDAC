...
load_camdac_opts_from_input <- function(sample_id, input_file, outdir, refdir){
  ...
  data = read.table(input_file) %>% dplyr::filter(V1==sample_id) %>% unlist %>% as.character
  opt=list()
...
load_cna_data_ascat <- function(tumour,config){
  ...
  result <- list(
    purity=purity, ploidy=ploidy, fit=fit, cna=cna_clean
  )
...
load_cna_data_battenberg <- function(tumour, config, bb_raw=FALSE, bb_dir=NA){
  ...
  bb_cna_fields = c("chr", "startpos", "endpos", "ntot", "nMaj1_A", "nMin1_A",
  ...
  result <- list(
    purity=pp$purity, ploidy=pp$ploidy, fit=pp$fit, cna=cna_clean
  )
...
chelper_import_pon_meth <- function(tumour, normal_id, config, pon_file){
  ...
  outdir = fs::path_dir(outfile); fs::dir_create(outdir)

...
allele_counts_to_beagle_vcf_ <- function(allele_counts, outfile, impute_legend, min_het_BAF=0.15){
  ...
  normal_counts = data.table::fread(allele_counts)
...
# Convert CAMDAC allele counts object to per-chromosome allele frequencies
create_allele_frequencies_chr <- function(camdac_allele_counts, outdir, sample_af_prefix, min_depth){
  ...
  dt = data.table::fread(camdac_allele_counts, select=c("chrom","POS","ref","alt","ref_counts","alt_counts"))[!is.na(POS)]
  # Filter for min depth
  dt = dt[ ref_counts + alt_counts >= min_depth ]
  # Split per chromosome for parallel processing
  dt_per_chrom = split(dt, dt$chrom); rm(dt)
...
filter_allele_frequencies_to_overlap <- function(outdir, tumour_prefix, normal_prefix){
  ...
  foreach(chrom=chroms) %dopar% {
    ...
    tumour_dt = data.table::fread(tumour_af)
    setkey(tumour_dt, "#CHR", "POS")
    normal_dt = data.table::fread(normal_af)
...
battenberg_wgbs_wrapper <- function(
                                tumourname,
                                normalname,
                                imputeinfofile,
                                problemloci,
  ...
  Battenberg::combine.baf.files(inputfile.prefix=paste(tumourname[sampleidx], "_chr", sep=""),
  ...
  Battenberg::fit.copy.number(samplename=tumourname[sampleidx],
  ...
  Battenberg::callSubclones(sample.name=tumourname[sampleidx],
